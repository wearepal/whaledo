...
class SwinTransformerBlock(nn.Module):
    ...
    def _calc_window_shift(
        ...
        window_size = [
            r if r <= w else w for r, w in zip(self.input_resolution, target_window_size)
        ]
        shift_size = [
            0 if r <= w else s
...
class SwinTransformerV2(nn.Module):
    r"""Swin Transformer V2
        A PyTorch impl of : `Swin Transformer V2: Scaling Up Capacity and Resolution`
            - https://arxiv.org/abs/2111.09883
    Args:
        img_size (int | tuple(int)): Input image size.  224
...
def _create_swin_transformer_v2(variant, pretrained=False, **kwargs):
    model = build_model_with_cfg(
        ...
        default_cfg=default_cfgs.get(variant, None),
        **kwargs,
    )
    return model
